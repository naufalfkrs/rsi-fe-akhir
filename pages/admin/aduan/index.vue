...
async function fetchKomplain() {
  try {
    const response = await axios.get(
      "http://127.0.0.1:8000/api/admin/komplain",
      {
        headers: {
    ...
    komplainList.value = response.data.data;
  } catch (error) {
...
const filteredKomplain = computed(() => {
  ...
  return komplainList.value.filter((k) => {
    if (activeFilter.value === "Belum Dilihat")
      return k.status_komplain === "Baru";
    if (activeFilter.value === "Sudah Dilihat")
      return k.status_komplain === "Selesai";
    return true;
  });
});
        ...
        <div class="overflow-x-auto">
          <table class="w-full bg-white border border-gray-200 rounded-lg">
            ...
            <tbody>
              ...
              <tr
                v-for="item in filteredKomplain"
                :key="item.id_komplain"
                ...
                <td class="px-6 py-4">
                  <span
                    :class="[
                      ...
                      getStatusClass(item.status_komplain),
                    ]"
                  >
